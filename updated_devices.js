...
const DOOR_OPEN = 's|FREE';
const DOOR_CLOSED = 's|OCCUPIED';

const BELL_OFF = 's|INWORK';
const BELL_ON = 's|FINISHED';
        ...
        switch (deviceId.replace(/\d/g, '')) {
            case 'door':
                ...
                if (state.s !== 'LOCKED') {
                    ...
                    state.s = getRandom() > rate ? 'FREE' : 'OCCUPIED';
                }
                setDeviceState(deviceId, toUltraLight(state), isSensor);
                break;
        }
    ...
    _.forEach(deviceIds, (deviceId) => {
        ...
        switch (deviceId.replace(/\d/g, '')) {
            ...
            case 'motion':
                ...
                if (getDoorState(deviceId, 'motion') === 'FREE') {
                    if (state.c === 1) {
                        state.c = 0;
                    } else {
            ...
            case 'lamp':
                if (state.s === 'OFF') {
                    state.l = 0;
                } else if (getDoorState(deviceId, 'lamp') === 'FREE') {
